...
yay -Syy ; yay -Syyu ; yay -Fy ; updatedb
echo

if ! command -v ansible &> /dev/null; then
    echo -e "${YELLOW}Installing Ansible.${NC}"
    yay -S --noconfirm ansible-core
    echo
fi

if ! command -v aws &> /dev/null; then
    echo -e "${YELLOW}Installing AWS.${NC}"
    yay -S --noconfirm awscli
    echo
fi

if ! command -v go &> /dev/null; then
    echo -e "${YELLOW}Installing Go.${NC}"
    yay -S --noconfirm golang-go
...
if ! command -v raven &> /dev/null; then
    ...
    yay -S --noconfirm raven
    echo
fi

if ! command -v sublist3r &> /dev/null; then
    echo -e "${YELLOW}Installing Sublist3r.${NC}"
    yay -S --noconfirm sublist3r
    echo
fi
...
if [ -d /opt/manspider/.git ]; then
    ...
    yay -S --noconfirm antiword tesseract-ocr
    echo
fi
